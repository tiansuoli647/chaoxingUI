...
class AI(Tiku):
    ...
    def _query(self, q_info: dict):
        ...
        system_prompts = {
            "single": "本题为单选题，你只能选择一个选项。请根据题目和选项回答问题，你必须从给出的选项列表中选择唯一正确的选项内容，并在 JSON 的 Answer 中原样输出该选项的完整内容，绝对不能进行任何修改、缩写、概括，也不能带有任何前缀（如 A.、B.等）或多余的标点符号。示例回答：{\"Answer\": [\"正确选项完整内容\"]}。除此之外绝对不要输出任何多余的内容，不要有任何解释说明，也不要使用 MD 语法。如果你使用了互联网搜索，也请不要返回搜索的结果和参考资料",
            "multiple": "本题为多选题，你必须选择两个或以上选项。请根据题目和选项回答问题，你必须从给出的选项列表中选择所有正确的选项，并在 JSON 的 Answer 列表中原样输出这些选项的完整内容，每个选项内容绝对不能进行任何修改、缩写、概括，也不能带有任何前缀（如 A.、B.等）或多余的标点符号。示例回答：{\"Answer\": [\"正确选项1完整内容\", \"正确选项2完整内容\"]}。除此之外绝对不要输出任何多余的内容，不要有任何解释说明，也不要使用 MD 语法。如果你使用了互联网搜索，也请不要返回搜索的结果和参考资料",
            "completion": "本题为填空题，你必须根据语境和相关知识填入合适的内容。请根据题目回答问题，并在 JSON 的 Answer 中输出正确的填空答案，示例回答：{\"Answer\": [\"答案\"]}。除此之外绝对不要输出任何多余的内容，不要输出题目或任何解释说明，也不要使用 MD 语法。如果你使用了互联网搜索，也请不要返回搜索的结果和参考资料",
            "judgement": "本题为判断题，你只能回答正确或者错误。请根据题目回答问题，你只能且必须在 JSON 的 Answer 中输出'正确'或'错误'，示例回答：{\"Answer\": [\"正确\"]}。除此之外绝对不要输出任何多余的内容，绝对不能带有任何解释说明或多余字样，也不要使用 MD 语法。如果你使用了互联网搜索，也请不要返回搜索的结果和参考资料",
            "shortanswer": "本题为简答题，你必须根据语境和相关知识填入合适的内容。请根据题目回答问题，并在 JSON 的 Answer 中输出精简的答案内容，示例回答：{\"Answer\": [\"这是我的答案\"]}。除此之外绝对不要输出任何多余的内容，也不要使用 MD 语法。如果你使用了互联网搜索，也请不要返回搜索的结果和参考资料"
        }
...
class SiliconFlow(Tiku):
    ...
    def _query(self, q_info: dict):
        ...
        # 构造系统提示词
        system_prompts = {
            "single": "本题为单选题，请根据题目和选项选择唯一正确答案。你必须从给出的选项列表中选择唯一正确的选项内容，并在 JSON 的 Answer 中原样输出该选项的完整内容，绝对不能进行任何修改、缩写、概括，也不能带有任何前缀（如 A.、B.等）或多余的标点符号。示例回答：{\"Answer\": [\"正确选项完整内容\"]}。除此之外绝对不要输出任何多余的内容，不要有任何解释说明，也不要使用 MD 语法。如果你使用了互联网搜索，也请不要返回搜索的结果和参考资料",
            "multiple": "本题为多选题，请选择所有正确选项。你必须从给出的选项列表中选择所有正确的选项，并在 JSON 的 Answer 列表中原样输出这些选项的完整内容，每个选项内容绝对不能进行任何修改、缩写、概括，也不能带有任何前缀（如 A.、B.等）或多余的标点符号。示例回答：{\"Answer\": [\"正确选项1完整内容\",\"正确选项2完整内容\"]}。除此之外绝对不要输出任何多余的内容，不要有任何解释说明，也不要使用 MD 语法。如果你使用了互联网搜索，也请不要返回搜索的结果和参考资料",
            "completion": "本题为填空题，请直接给出填空内容。请直接给出填空答案内容，并在 JSON 的 Answer 中输出，示例回答：{\"Answer\": [\"答案文本\"]}。除此之外绝对不要输出任何多余的内容，不要输出题目或任何解释说明，也不要使用 MD 语法。如果你使用了互联网搜索，也请不要返回搜索的结果和参考资料",
            "judgement": "本题为判断题，请回答'正确'或'错误'。你只能且必须在 JSON 的 Answer 中输出'正确'或'错误'，示例回答：{\"Answer\": [\"正确\"]}。除此之外绝对不要输出任何多余的内容，绝对不能带有任何解释说明或多余字样，也不要使用 MD 语法。如果你使用了互联网搜索，也请不要返回搜索的结果和参考资料",
            "shortanswer": "本题为简答题，请根据题目给出回答内容。请根据题目给出精简的回答内容，并在 JSON 的 Answer 中输出，示例回答：{\"Answer\": [\"回答文本\"]}。除此之外绝对不要输出任何多余的内容，不要输出题目或任何解释说明，也不要使用 MD 语法。如果你使用了互联网搜索，也请不要返回搜索的结果和参考资料"
        }
